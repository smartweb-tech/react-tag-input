...
class AutocompleteTextField extends React.Component {
  ...
  getAutocompleteData=()=>{
    ...
    const helperOptions = options.slice(0, optionNumber).map((val, idx) => {
      ...
      let item = {
        ...
        displayLabel: val.slice(0, highlightStart),
        smallLabel: val.substr(highlightStart, matchLength),
        lastLabel: val.slice(highlightStart + matchLength)
      };
      res.options.push(item);
    });

    res.status = true;
    res.className = "react-autocomplete-input";
    res.style = { left: left + offsetX, top: top + offsetY };
  ...
  handleRenderAutocompleteList(){
    // if( this.props.enableRenderAutocomplete === true ){
    //   let dataJson = this.getAutocompleteData();
    //   this.props.onRenderAutocompleteList(dataJson);
    //   return null;
    // }
...
const AutoCompleteList = ({ data, ...props }) => {
  ...
  return (
    <div>
      <ul className={data.className} style={data.style} >
        {
...
export { AutocompleteTextField, AutoCompleteList };
